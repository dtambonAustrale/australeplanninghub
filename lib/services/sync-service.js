...
/**
 * Sync a batch of sessions — sequential by default (concurrency = 1) to avoid
 * rate-limiting (503) on Digiforma's API. Retries are handled at the HTTP level
 * in digiforma-client.js. A 500ms pause is added between each session.
 */
export async function syncBatch(syncRunId, sessionIds, options = {}) {
  const concurrency = options.concurrency || 1;
  const pauseMs = options.pauseMs ?? 500; // pause between sessions
  const errors = [];
  let syncedSessions = 0;

  // Process in groups of `concurrency` (default 1 = fully sequential)
  for (let i = 0; i < sessionIds.length; i += concurrency) {
    const group = sessionIds.slice(i, i + concurrency);

    const results = await Promise.allSettled(
      group.map(async (sessionId) => {
        const details = await fetchTrainingSessionDetails(sessionId);
        if (!details) throw new Error('Session details not found');
    ...
    results.forEach((result, idx) => {
      if (result.status === 'fulfilled') {
        ...
        const sessionId = group[idx];
        console.error(`[SyncBatch] Error syncing session ${sessionId}:`, result.reason?.message);
        errors.push({ sessionId, error: result.reason?.message || 'Unknown error' });
      }
    });

    // Pause between groups to avoid rate limiting
    if (i + concurrency < sessionIds.length) {
      await sleep(pauseMs);
    }
  }
...
export async function syncTrainingSession(sessionDetails) {
  ...
  if (slots.length > 0) {
    ...
    // Collect all extra trainees found in customerTrainees across all slots (batch upsert once)
    const extraTraineesMap = new Map();
    for (const slot of slots) {
      for (const ct of slot.customerTrainees || []) {
        if (ct.trainee?.id) {
          extraTraineesMap.set(String(ct.trainee.id), ct.trainee);
        }
      }
    }
    if (extraTraineesMap.size > 0) {
      await supabaseAdmin
        .from('trainees')
        .upsert([...extraTraineesMap.values()].map(mapTrainee), { onConflict: 'id' });
    }

    // Build slot_trainees rows
