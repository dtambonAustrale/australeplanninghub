...
export async function getPlanningResults(filters = {}) {
  ...
  if (filters.year) {
    ...
    // Include rows where date is NULL (not yet set) OR falls within the year
    query = query
      .or(`session_start_date.is.null,session_start_date.gte.${start}`)
      .or(`session_end_date.is.null,session_end_date.lte.${end}`);
  }
